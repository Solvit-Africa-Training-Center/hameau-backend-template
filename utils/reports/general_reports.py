...
def generate_child_progress_pdf(child, current_progress, previous_progress=None):
    ...
    elements.append(
        Paragraph(f"<b>Date of Birth:</b> {child.date_of_birth}", styles["Normal"])
    )
    ...
    elements.append(
        Paragraph(
            f"<b>Current Progress ({current_progress.created_on.strftime('%Y-%m-%d')}):</b>",
            styles["Heading2"],
        )
    )
    ...
    if previous_progress:
        elements.append(
            Paragraph(
                f"<b>Previous Progress ({previous_progress.created_on.strftime('%Y-%m-%d')}):</b>",
                styles["Heading2"],
            )
        )
        ...
        elements.append(
            Paragraph(
                "<b>Previous Progress:</b> No previous records found (First Report).",
                styles["Normal"],
            )
        )
    ...
    def render(self, data, accepted_media_type=None, renderer_context=None):
        ...
        if "latest_progress" in data:
            buffer = generate_child_progress_pdf(
                data["child"], data["latest_progress"], data.get("previous_progress")
            )
        else:
